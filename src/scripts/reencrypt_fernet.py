...
def _descifrar(valor: bytes, fernet_old: Any) -> Optional[bytes]:
    ...
    try:
        ...
        logger.warning("Token Fernet inválido durante descifrado")
        return None
    except Exception as exc:
        logger.error("Error criptográfico inesperado durante descifrado: %s", exc)
        return None
    ...
    try:
        ...
        for pk_valor, valor_cifrado in filas:
            ...
            try:
                ...
                logger.error("Error al cifrar con la nueva clave: %s", enc_exc)
                omitidas += 1
                continue
        ...
        if not dry_run:
            try:
                ...
                logger.error("Fallo al ejecutar rollback de base de datos: %s", rb_exc)
        return ResultadoRecifrado(
            tabla=tabla, campo=campo,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        logger.error("Error general durante la ejecución: %s", exc)
        db.close()
        return 1
