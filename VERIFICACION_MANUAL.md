...
## ADICIONES DE SESIÓN 2026-06-20/21 — resultados reales verificados

### check_coverage.py (04 §3.3) — resultado real
```bash
python scripts/check_coverage.py coverage.xml
```
Resultado 2026-06-21 (820 tests · `--cov=src --cov=api`):
```
catalogo (domain)   branch    100.0%   90.0%  ✅
pedidos  (domain)   branch     98.5%   90.0%  ✅
stock    (domain)   branch    100.0%   95.0%  ✅
taller   (domain)   branch     98.1%   85.0%  ✅
shared              branch     92.9%   80.0%  ✅
api (routes)        branch     87.5%   80.0%  ✅
infra (line todos)  line       98.9%   70.0%  ✅
RESULTADO: OK — todos los criterios cumplen su umbral.
```

### Auth middleware JWT RS256 (07 §2, §3.2)
```bash
pytest tests/unit/auth/test_auth.py -v
```
**Resultado:** 10 passed — verifica: 401 sin token · 401 token inválido · 401 token expirado ·
401 firma incorrecta · 403 VENDEDOR en endpoint ADMIN · 200 ADMINISTRADOR · 200 SUPERADMIN ·
200 endpoint público sin token.

`require_roles()` aplicado en 32 endpoints restringidos de 03 §6.2-§6.6.

### EP-AUTH-01 a EP-AUTH-04 y EP-ADM-01 a EP-ADM-05 (03 §6.6)
```bash
pytest tests/integration/test_auth_admin_endpoints.py -v
```
**Resultado:** 25 passed — cubre login/mfa/refresh/logout + parametros/vehiculos/mecanicos/usuarios
con tests de roles 401/403/201 según roles exactos de 03 §6.6.

### tests/integration — tests de API sin PostgreSQL (corrección)
Los tests `test_api_*.py` usan `httpx.AsyncClient + ASGITransport + repos in-memory`:
NO requieren PostgreSQL real. Resultados reales: 18+33+27+34 = **112 passed** sin infra.

### seed nivel 1 — resultados reales con inserción verificada
```
python scripts/seed.py --level=1 --module=pedidos --env=test
→ pedidos nivel 1: 3 pedidos sembrados (2 clientes, estados: BORRADOR/CONFIRMADO/CANCELADO)

python scripts/seed.py --level=1 --module=stock --env=test
→ stock nivel 1: 5 stock_repuesto sembrados (1 agotado, 1 reabastecimiento pendiente)

python scripts/seed.py --level=1 --module=taller --env=test
→ taller nivel 1: 1 mecanico, 2 vehiculos, 2 ordenes_trabajo sembrados (estados: ABIERTA)
```

### Total de tests — 2026-06-21
```bash
python -m pytest tests/ --co -q
```
**820 tests collected** · 0 fallos · `check_coverage exit 0`.
